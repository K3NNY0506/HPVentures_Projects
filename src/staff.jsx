...
function Staff() {
	...
	const normalizeEmployees = (list) =>
		(list || []).map((employee) => ({
			...employee,
			imagePosition: {
				x: Number(employee?.imagePosition?.x ?? 50),
				y: Number(employee?.imagePosition?.y ?? 50),
			},
		}))
	...
	useEffect(() => {
		...
		const refreshEmployees = async () => {
			...
			const normalized = normalizeEmployees(loadedEmployees)
			setEmployees(normalized)
			setSelectedDepartment('All')

			// Temporary debug – remove after confirming
			console.log(
				'Staff employees imagePosition:',
				normalized.map((e) => ({
					name: e.name,
	...
	useEffect(() => {
		...
		const measure = () => {
			...
			setSelectorOffset((currentOffset) =>
				Math.min(currentOffset, Math.max(0, (selectorTrackRef.current?.scrollWidth || 0) - visibleWidth))
			)
		}
		measure()
		...
		setSelectorOffset((currentOffset) => {
			if (direction > 0) {
				...
				const nextButton = buttons.find(
					(button) => button.offsetLeft + button.offsetWidth / 2 > currentCenter + 1
				)
			...
			const previousOffset =
				previousButton.offsetLeft + previousButton.offsetWidth / 2 - selectorVisibleWidth / 2
	...
	useEffect(() => {
		...
		if (centeredButton && centeredButton.button.textContent !== selectedDepartment) {
			setSelectedDepartment(centeredButton.button.textContent)
		}
	...
	const employeeRows = Array.from({ length: Math.ceil(filteredEmployees.length / 5) }, (_, rowIndex) => {
		const rowEmployees = filteredEmployees
			.slice(rowIndex * 5, rowIndex * 5 + 5)
			.map((employee, offset) => ({ employee, index: rowIndex * 5 + offset }))
		return rowEmployees
	})
		...
		<main className="site-shell staff-page">
			<nav className={`topbar ${headerScrolled ? 'scrolled' : 'transparent'}`} aria-label="Primary navigation">
				...
				<div className={menuOpen ? 'nav-links open' : 'nav-links'}>
					{['About us', 'Groups', 'The Team', 'Careers'].map((item) => (
						<a
							href={
								item === 'Groups'
									? '/groups'
				...
				<a className="phone-link" href="#">
					<span aria-hidden="true">☎</span> (032) 343-9651
				</a>
			</nav>
			...
			<section className="staff-content">
				<div className="staff-section-heading"></div>
				...
				<div className="category-selector-wrap">
					{staffDepartments.length > 5 && (
						<button
							...
							onClick={() => scrollDepartmentSelector(-1)}
							disabled={selectorOffset === 0}
						>
							‹
						</button>
					)}
				...
				<div className="staff-layout">
					<div className="employee-grid">
						{employeeRows.map((row, rowIndex) => (
							<div className="employee-row" key={rowIndex}>
								{row.map(({ employee, index }) => {
									const posX = employee.imagePosition?.x ?? 50
									const posY = employee.imagePosition?.y ?? 50
									const positionValue = `${posX}% ${posY}%`

									return (
										<Fragment key={`${employee.id || employee.name}-${posX}-${posY}-${index}`}>
											<div
												className="employee-card"
												data-column={index % 5}
												data-mobile-column={index % 2}
												data-focal={`${posX},${posY}`}
												style={
													employee.image
														? {
